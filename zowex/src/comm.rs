...
/**
 * Perform the identity handshake with the daemon, before anything is sent. 
 * We require the other end to first prove it knows the secret token from
 * our owner-only pid file, bound to a nonce that we alone chose, before we
 ...
 *      On failure (including a mismatched, missing, or malformed proof), an error result. 
 *      Callers must not send anything further on this connection when this fails.
 */
pub async fn comm_handshake(stream: &mut DaemonClient, token: &str) -> io::Result<String> {
