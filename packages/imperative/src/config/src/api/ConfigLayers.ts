...
export class ConfigLayers extends ConfigApi {
    ...
    public write(opts?: { user: boolean; global: boolean }) {
        ...
        // writeFileSync only applies a mode when it creates the file, so whether the
        // file is new has to be known before the write happens.
        const fileIsNew = !fs.existsSync(layer.path);
        const fileMode = ConfigLayers.modeForNewFile(layer);

        // Write the layer
        try {
            /* Create the file with restricted permissions before any content goes into
             * it, so the content is never readable by other accounts, not even briefly.
             * Passing `mode` to writeFileSync would not be equivalent: writeFileSync
             * silently ignores `mode` for a file that already exists, which hides the
             * fact that a mode can only ever be applied at creation time. Creating the
             * file here makes that "new files only" rule explicit.
             */
        ...
        // A file that already existed keeps whatever mode it had. Rather than silently
        // tightening a mode the user may have chosen, warn when that mode exposes a
        // credential to other accounts.
        if (!fileIsNew) {
    ...
    private static modeForNewFile(layer: IConfigLayer): number | undefined {
        ...
        // This is the project team layer, which teams share. Keep the umask default.
        return undefined;
    }
