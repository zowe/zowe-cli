...
export class Imperative {
    ...
    /**
    @deprecated DEFAULT_DEBUG_FILE is no longer used.
                We cannot remove it because it is public. It should be removed in V4.
    */
    public static readonly DEFAULT_DEBUG_FILE = join(process.cwd(), "imperative_debug.log");

    private static readonly DEBUG_FILE_NAME = "imperative_debug.log";
    ...
    public static async init(config?: IImperativeConfig): Promise<void> {
        try {
            ...
            if (error?.suppressDump) {
                ...
                if (ImperativeConfig.instance?.loadedConfig?.defaultHome) {
                    // put debug file in CLI home logs directory when ImperativeConfig has been
                    // sufficiently defined by the time of the exception
                    const zoweLogsDir = join(ImperativeConfig.instance.cliHome, "logs");
                    if (fs.existsSync(zoweLogsDir)) {
                        debugFilePath = join(zoweLogsDir, Imperative.DEBUG_FILE_NAME);
                    } else {
                        // When no .zowe/logs directory, fall back to user's home directory
                        debugFilePath = join(os.homedir(), Imperative.DEBUG_FILE_NAME);
                    }
                } else {
                    // When no ImperativeConfig, fall back to user's home directory
                    debugFilePath = join(os.homedir(), Imperative.DEBUG_FILE_NAME);
                }
