...
import { syntaxErrorHeader } from "../../messages/CoreMessages";
...
export class SyntaxValidator {
    ...
    private validateSyntax(commandArguments: ICommandArguments, responseObject: CommandResponse): boolean {
         ...
         */
        if (this.mCommandDefinition.positionals?.length > 0) {
            ...
            for (const positional of this.mCommandDefinition.positionals) {
                if (commandArguments[positional.name] != null) {
                    if (positional.regex) {
                        if (commandArguments[positional.name].toString().match(new RegExp(positional.regex)) == null) {
                            valid = false;
                    ...
                    if (positional.stringLengthRange?.[0] != null && positional.stringLengthRange?.[1]) {
        ...
        for (const optionName of Object.keys(optionDefs)) {
             ...
             */
            if (optionDef.absenceImplications?.length > 0) {
            ...
            if (util.optionWasSpecified(optionName, this.mCommandDefinition, commandArguments)) {
                ...
                if (optionDef.conflictsWith?.length > 0) {
                 ...
                 */
                if (optionDef.impliesOneOf?.length > 0) {
                 ...
                 */
                if (optionDef.numericValueRange?.[0] != null && optionDef?.numericValueRange?.[1] != null) {
                 ...
                 */
                if (optionDef.stringLengthRange?.[0] != null && optionDef.stringLengthRange?.[1] != null) {
    ...
    private getMustacheSummaryForOption(optionDefinition: ICommandOptionDefinition | ICommandPositionalDefinition,
        ...
        if (!isPositional) {
            ...
            aliasString = (def.aliases?.length ?? -1) <= 0 ? "" : "(" + def.aliases.map((alias: string) => {
