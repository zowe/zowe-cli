...
describe("receive TSO app handler behavior", () => {
    ...
    it("should properly receive and parse data from receive TSO response", async () => {
        ...
        const sendSpy = jest.spyOn(AddressSpaceApps,"send");
        const handler = new SendASAppHandler.default();
        const params = Object.assign({}, ...[DEFAULT_PARAMETERS]);
        let error = undefined;
        params.arguments = {...params.arguments,account: "izuacct", appKey: "test2", servletKey: "ZOWEUSER-129-aaceaaap", message: "LONG 100"};
        try{
