...
describe("ApimlAutoInitHandler", () => {
    ...
    it("should not have changed - user & password", async () => {
        ...
        expect(response.profiles.my_base.secure).toContain("tokenValue");
        expect(response.profiles.my_base.properties.tokenType).toEqual(SessConstants.TOKEN_TYPE_APIML);
        expect(response.profiles.my_base.properties.tokenValue).toEqual("fakeToken");
    });
    ...
    it("should not have changed - user & password with existing base profile", async () => {
        ...
        expect(mockConvertApimlProfileInfoToProfileConfig).toHaveBeenCalledTimes(1);
        expect(mockLogin).toHaveBeenCalledTimes(0);
        expect(response.profiles.my_base.secure).not.toContain("tokenValue");
        expect(response.profiles.my_base.properties.tokenType).not.toBeDefined();
        expect(response.profiles.my_base.properties.tokenValue).not.toBeDefined();
    });
