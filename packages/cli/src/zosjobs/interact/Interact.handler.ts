...
import OutputHandler from "../download/download-output/Output.handler";
...
export default class InteractiveJobsHandler extends ZosmfBaseHandler {
    ...
    public async processCmd(params: IHandlerParameters): Promise<void> {
        ...
        switch (jobActions[jobActionIndex - 1]) {
            ...
            case "Download": {
                const jobid: string = jobs[jobIndex - 1].jobid;
                const downloadHandler: any = new OutputHandler();
                await downloadHandler.processCmd.call(this, {
                    ...params,
                    arguments: {
                        ...params.arguments,
                        jobid,
            ...
            case "Cancel": {
                ...
                await CancelJobs.cancelJobForJob(this.mSession, job, params.arguments.modifyVersion);
            ...
            case "Delete": {
                ...
                await DeleteJobs.deleteJobForJob(this.mSession, job, params.arguments.modifyVersion);
            ...
            default: {
                throw new ImperativeError({ msg: "Unknown action selected" });
            }
        }
    }
}
