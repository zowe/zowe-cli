...
import { IHandlerParameters, TextUtils } from "npm:@zowe/imperative";
import {
...
export default class CatalogTemplatesHandler extends ZosmfBaseHandler {
    public async processCmd(commandParameters: IHandlerParameters) {
        const templates: IPublishedTemplates = await ListCatalogTemplates.listCatalogCommon(
            this.mSession,
        ...
        if (commandParameters.arguments.allInfo) {
            prettifiedTemplates = TextUtils.explainObject(
                templates,
                explainPublishedTemplatesFull,
                true
            );
