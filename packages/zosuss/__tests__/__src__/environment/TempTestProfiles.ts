...
export class TempTestProfiles {
    ...
    private static async createSshProfile(testEnvironment: ITestEnvironment): Promise<string> {
        const profileName: string = "tmp_ssh_" + uuidv4().substring(0, TempTestProfiles.MAX_UUID_LENGTH);
        ...
        const createProfileScript = TemporaryScripts.SHEBANG +
            `${TemporaryScripts.ZOWE_BIN} profiles create ssh ${profileName} --user ${sshProperties.user} --pass ` +
