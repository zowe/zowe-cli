...
JOBID1=`zowe zos-jobs submit data-set $JCL --host $HOST --port $PORT --user $USER --pass $PASS --ru=false --rff jobid --rft string`
JOBID2=`zowe zos-jobs submit data-set $JCL --host $HOST --port $PORT --user $USER --pass $PASS --ru=false --rff jobid --rft string`

echo "Listing jobs to find job IDs $JOBID1 and $JOBID2"

LIST_JOB_OUTPUT=`zowe zos-jobs list jobs --host $HOST --port $PORT --user $USER --pass $PASS --ru=false`
