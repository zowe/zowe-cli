...
describe("zosmf check status", () => {
    ...
    describe("Expected failures", () => {
        ...
        it("should fail due to invalid port", async () => {
            // create a temporary bright profile with an invalid port
            const scriptPath = testEnvironment.workingDir + "_create_profile_invalid_port";
            const command = "bright profiles create zosmf " + host + "temp --host " + host + " --port " + port + 1
            ...
            await IO.writeFileAsync(scriptPath, "bright profiles set  zosmf " + host + "temp");
