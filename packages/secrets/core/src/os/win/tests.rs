...
mod tests {
    ...
    // ============================================================================
    // Integration-style Tests (require actual certificates)
    // ============================================================================
    
    // Note: These tests are commented out because they require actual certificates
    // in the Windows Certificate Store. Uncomment and modify for integration testing.
    
    /*
    #[test]
    #[ignore] // Requires actual certificate in store
    fn test_get_certificate_with_real_cert() {
        let service = String::from("TestService");
        let account = String::from(TEST_CERT_SUBJECT);
        
        let result = get_certificate(&service, &account);
        match result {
            Ok(Some(cert_data)) => {
                assert!(!cert_data.is_empty(), "Certificate data should not be empty");
                // Verify it's valid DER format (starts with 0x30)
                assert_eq!(cert_data[0], 0x30, "Should be valid DER format");
            }
            Ok(None) => {
                panic!("Certificate should exist for subject: {}", TEST_CERT_SUBJECT);
            }
            Err(e) => {
                panic!("Failed to get certificate: {:?}", e);
            }
        }
    }

    #[test]
    #[ignore] // Requires actual certificate with non-exportable key
    fn test_create_and_use_identity_context() {
        let service = String::from("TestService");
        let account = String::from(TEST_CERT_SUBJECT);
        
        // Create identity context
        let handle_id = match create_identity_context(&service, &account) {
            Ok(Some(id)) => id,
            Ok(None) => panic!("Certificate not found: {}", TEST_CERT_SUBJECT),
            Err(e) => panic!("Failed to create identity context: {:?}", e),
        };
        ...
        let signature = match sign_with_identity(&handle_id, &algorithm, &data) {
            Ok(Some(sig)) => sig,
            Ok(None) => panic!("Signing returned None"),
            Err(e) => panic!("Failed to sign data: {:?}", e),
        };
        
        assert!(!signature.is_empty(), "Signature should not be empty");
        
        // Release identity context
        match release_identity_context(&handle_id) {
            Ok(true) => {
                // Successfully released
            }
            Ok(false) => {
                panic!("Failed to release identity context");
            }
            Err(e) => {
                panic!("Error releasing identity context: {:?}", e);
            }
        }
    }
    ...
    fn test_identity_lifecycle() {
        ...
        // Create identity
        let handle_id = create_identity_context(&service, &account)
            .expect("Should create identity")
            .expect("Certificate should exist");
        
        // Verify we can retrieve it
        let identity = identity_cache::get_cached_identity(&handle_id)
            .expect("Should retrieve cached identity");
        
        assert!(!identity.cert_context.is_null(), "Certificate context should not be null");
        assert_ne!(identity.key_handle, 0, "Key handle should not be zero");
        
        // Release identity
        let released = release_identity_context(&handle_id)
            .expect("Should release identity");
        assert!(released, "Should successfully release");
        
        // Verify it's gone
        let result = identity_cache::get_cached_identity(&handle_id);
        assert!(result.is_err(), "Identity should no longer be cached");
    }
    */
}
