...
pub fn create_tls_pipe(
    ...
    // 1. Get cert context using schannel's CertStore
    let store = CertStore::open_current_user("MY")
        .map_err(|e| KeyringError::Os(format!("Failed to open MY cert store: {}", e)))?;
    
    let mut cert_context: Option<CertContext> = None;
    for cert in store.certs() {
        if let Ok(subject) = cert.subject_name() {
            if subject.contains(cert_account) {
                cert_context = Some(cert);
                break;
            }
        }
    }
    
    let cert = cert_context.ok_or_else(|| KeyringError::Os(format!("Certificate not found for account: {}", cert_account)))?;
            ...
            thread::spawn(move || {
                ...
                tls_builder.verify_callback(move |validation_result| {
                    if reject_unauthorized {
                        validation_result.result().is_ok()
                    } else {
                        true
                    }
                });
                
                let mut tls_stream = match tls_builder.connect(creds, &host_name, remote_stream) {
                    Ok(s) => s,
