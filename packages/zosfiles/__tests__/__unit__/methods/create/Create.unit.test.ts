        ...
        describe("Success scenarios", () => {
            ...
            it("should be able to create a sequential data set (PS) with responseTimeout", async () => {
                ...
                expect(mySpy).toHaveBeenCalledWith(
                    dummySession,
                    endpoint,
                    expect.arrayContaining([ZosmfHeaders.ACCEPT_ENCODING, { [ZosmfHeaders.X_IBM_RESPONSE_TIMEOUT]: "5" }]),
                    JSON.stringify({
                        ...CreateDefaults.DATA_SET.SEQUENTIAL,
                        ...dsOptions,
                        ...{
            ...
            it("should be able to create a classic data set and override multiple options", async () => {
                ...
                expect(response.success).toBe(true);
                expect(response.commandResponse).toContain("created successfully");
                expect(mySpy).toHaveBeenCalledWith(dummySession, endpoint, expect.arrayContaining([ZosmfHeaders.ACCEPT_ENCODING]), JSON.stringify({
        ...
        it("should add responseTimeout header when supplied in Create.zfs", async () => {
            ...
            expect(mySpy).toHaveBeenCalledWith(
                ...
                expect.arrayContaining(
                    [{"X-IBM-Response-Timeout": "5"}, {"Accept-Encoding": "gzip"}, {"Content-Length": String(jsonContent.length)}]
                ),
