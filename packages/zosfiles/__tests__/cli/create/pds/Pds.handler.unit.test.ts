...
describe("Create PDS data set handler", () => {
    describe("process method", () => {
        ...
        it("should create a PDS data set with volser set", async () => {
            // Require the handler and create a new instance
            const handlerReq = require("../../../../src/cli/create/pds/Pds.handler");
            const handler = new handlerReq.default();
            const dataSetName = "testing";
            ...
            expect(profFunc).toHaveBeenCalledWith("zosmf", false);
            expect(Create.dataSet).toHaveBeenCalledTimes(1);
            expect(Create.dataSet).toHaveBeenCalledWith(fakeSession, CreateDataSetTypeEnum.DATA_SET_PARTITIONED, dataSetName, { volser: "abcdef"});
            expect(jsonObj).toMatchSnapshot();
            expect(apiMessage).toMatchSnapshot();
            expect(logMessage).toMatchSnapshot();
        });
