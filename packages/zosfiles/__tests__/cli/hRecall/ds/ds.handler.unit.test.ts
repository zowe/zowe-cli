...
describe("DsHandler", () => {
    ...
    it("should call HRecall.dataSet with options", async () => {
        const handler = new DSHandler();

        expect(handler).toBeInstanceOf(ZosFilesBaseHandler);
        const options: IRecallOptions = { "request": "hrecall", "wait": true, responseTimeout: 5 };
        ...
        const expectedOptions: IRecallOptions = { "request": "hrecall", "wait": true, responseTimeout: 5 };
