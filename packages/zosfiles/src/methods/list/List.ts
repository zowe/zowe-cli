...
export class List {
            ...
            if (options.responseTimeout != null) {
                reqHeaders.push({
                    [ZosmfHeaders.X_IBM_RESPONSE_TIMEOUT]:
                        options.responseTimeout.toString(),
                });
            }

            this.log.debug(`Endpoint: ${endpoint}`);

            const data = await ZosmfRestClient.getExpectString(
                session,
                endpoint.concat(params.size > 0 ? `?${params.toString()}` : ""),
                reqHeaders
            );
            let response: any;
            try {
                ...
                if (match != null) {
                    ...
                    const lineNum =
                        data.slice(0, parseInt(match[1])).split("\n").length -
                        1;
                    ...
                    const invalidMemberCount =
                        response.returnedRows - response.items.length;
                    this.log.warn(
                        `${invalidMemberCount} members failed to load due to invalid name errors for ${dataSetName}`
                    );
                } else {
                    throw err;
                }
     ...
     * @example
     */
    public static async membersMatchingPattern(
        session: AbstractSession,
        dataSetName: string,
        patterns: string[],
        ...
        ImperativeExpect.toNotBeNullOrUndefined(
            patterns,
            ZosFilesMessages.missingPatterns.message
        );
        ...
        if (zosmfResponses.length === 0) {
            return {
                success: false,
                commandResponse:
                    ZosFilesMessages.noMembersMatchingPattern.message,
                apiResponse: [],
            };
        }
        ...
        for (const pattern of options.excludePatterns || []) {
            ...
            response.apiResponse.items.forEach(
                (membersObj: IZosmfListResponse) => {
                    const responseIndex = zosmfResponses.findIndex(
                        (response) => response.member === membersObj.member
                    );
                    if (responseIndex !== -1) {
        ...
        return {
            ...
            commandResponse: util.format(
                ZosFilesMessages.membersMatchedPattern.message,
                zosmfResponses.length
            ),
    ...
    public static async dataSet(
        ...
        try {
            let endpoint = posix.join(
                ZosFilesConstants.RESOURCE,
                `${ZosFilesConstants.RES_DS_FILES}?${
                    ZosFilesConstants.RES_DS_LEVEL
                }=${encodeURIComponent(dataSetName)}`
            );
            if (options.volume) {
                endpoint = `${endpoint}&volser=${encodeURIComponent(
                    options.volume
                )}`;
            }
            if (options.start) {
                endpoint = `${endpoint}&start=${encodeURIComponent(
                    options.start
                )}`;
            }
            ...
            const response: any = await ZosmfRestClient.getExpectJSON(
                session,
                endpoint,
                reqHeaders
            );
    ...
    public static async fileList(
        ...
        if (
            options.depth ||
            options.filesys != null ||
            options.symlinks != null
        ) {
            if (
                !(
                    options.group ||
                ...
                throw new ImperativeError({
                    msg: ZosFilesMessages.missingRequiredTableParameters
                        .message,
                });
            }
        }
        ...
        try {
            ...
            if (options.group) {
                endpoint += `&${
                    ZosFilesConstants.RES_GROUP
                }=${encodeURIComponent(options.group)}`;
            }
            if (options.user) {
                endpoint += `&${
                    ZosFilesConstants.RES_USER
                }=${encodeURIComponent(options.user)}`;
            }
            if (options.name) {
                endpoint += `&${
                    ZosFilesConstants.RES_NAME
                }=${encodeURIComponent(options.name)}`;
            }
            if (options.size) {
                endpoint += `&${
                    ZosFilesConstants.RES_SIZE
                }=${encodeURIComponent(options.size)}`;
            }
            if (options.mtime) {
                endpoint += `&${
                    ZosFilesConstants.RES_MTIME
                }=${encodeURIComponent(options.mtime)}`;
            }
            if (options.perm) {
                endpoint += `&${
                    ZosFilesConstants.RES_PERM
                }=${encodeURIComponent(options.perm)}`;
            }
            if (options.type) {
                endpoint += `&${
                    ZosFilesConstants.RES_TYPE
                }=${encodeURIComponent(options.type)}`;
            }
            if (options.depth) {
                endpoint += `&${
                    ZosFilesConstants.RES_DEPTH
                }=${encodeURIComponent(options.depth)}`;
            }
            if (options.filesys != null) {
                if (options.filesys === true) {
                    endpoint += `&${ZosFilesConstants.RES_FILESYS}=all`;
                } else {
                    endpoint += `&${ZosFilesConstants.RES_FILESYS}=same`;
                }
            }
            if (options.symlinks != null) {
                if (options.symlinks === true) {
                    endpoint += `&${ZosFilesConstants.RES_SYMLINKS}=report`;
                } else {
                    endpoint += `&${ZosFilesConstants.RES_SYMLINKS}=follow`;
                }
            }
    ...
    public static async fs(
        ...
        try {
            ...
            if (options.fsname) {
                endpoint = posix.join(
                    endpoint,
                    `?${ZosFilesConstants.RES_FSNAME}=${encodeURIComponent(
                        options.fsname
                    )}`
                );
            }
    ...
    public static async fsWithPath(
        session: AbstractSession,
        options: IFsOptions = {}
    ): Promise<IZosFilesResponse> {
        try {
            ...
            if (options.path) {
                endpoint = posix.join(
                    endpoint,
                    `?${ZosFilesConstants.RES_PATH}=${encodeURIComponent(
                        options.path
                    )}`
                );
            }
     ...
     * ```
     */
    public static async dataSetsMatchingPattern(
        session: AbstractSession,
        patterns: string[],
        options: IDsmListOptions = {}
    ): Promise<IZosFilesResponse> {
        ...
        for (const pattern of patterns) {
            ...
            try {
                ...
                if (
                    !(
                        err instanceof ImperativeError &&
                        err.errorCode?.toString().startsWith("5")
                    )
                ) {
                    throw err;
                }
                    ...
                    if (options.task != null) {
                        options.task.percentComplete = Math.floor(
                            TaskProgress.ONE_HUNDRED_PERCENT *
                                (listsInitiated /
                                    response.apiResponse.items.length)
                        );
                        listsInitiated++;
                    }
                ...
                if (maxConcurrentRequests === 0) {
                    await Promise.all(
                        response.apiResponse.items.map(createListPromise)
                    );
                } else {
                    await asyncPool(
        ...
        if (zosmfResponses.length === 0) {
            return {
                success: false,
                commandResponse:
                    ZosFilesMessages.noDataSetsMatchingPattern.message,
                apiResponse: [],
            };
        }
        ...
        for (const pattern of options.excludePatterns || []) {
            ...
            response.apiResponse.items.forEach(
                (dataSetObj: IZosmfListResponse) => {
                    const responseIndex = zosmfResponses.findIndex(
                        (response) => response.dsname === dataSetObj.dsname
                    );
                    if (responseIndex !== -1) {
                        zosmfResponses.splice(responseIndex, 1);
                    }
        ...
        return {
            success: true,
            commandResponse: util.format(
                ZosFilesMessages.dataSetsMatchedPattern.message,
                zosmfResponses.length
            ),
            apiResponse: zosmfResponses,
        };
