...
imperative-test-cli config set "profiles.base.properties.user" "$baseUser" --user-config
if [ $CMDRC -gt 0 ]
then
...
imperative-test-cli config set "profiles.base.properties.password" "$basePass" --user-config
if [ $CMDRC -gt 0 ]
then
